...
imhere.GoogleMapPage = (function() {
	...
	var replyTwitter = (function(follower) {
		...
		alertify.prompt("Reply", function (e, str) {
		    ...
		    if (e) {
		        // user clicked "ok"
				imhereglobalcontext.twitterAPI.updateStatus(str, "", follower.id).then(
					function(d, dt){
					...
					function(e){
						...
						if(e.responseText!==undefined){
							...
							if(errorObj.errors[0].code==186){
								alertify.error("文字数が140文字を超えています");
							} else {
    ...
    return {
		...
		reloadModule: function() {
			var alertflg = false;
			// App起動時にGoogleMapJSが読み込めなかった場合
			try {
				google.map;
			} catch(e) {
				alertflg = true;
				alertify.alert("Google Mapを読み込めません。オンラインの状態で、アプリケーションを再起動してください。", function(e) {
						backMainPage();
						return;
					}
				);
			...
			if (alertflg == false) {
				...
				imhereglobalcontext.twitterAPI.searchAllTweets().then(
					...
					function(e){
						var undefined;
						if(e.responseText===undefined){
							alertify.error("オフラインのためマップ呼び出しに失敗しました");
						}else{
							alertify.error("予期せぬエラーのためマップ呼び出しに失敗しました");
						}
					}
				);
			}
